...
void selectionSort (int theArray[], unsigned int size) {
    ...
    for (unsigned int i = 0; i < size; ++i) {
        unsigned int smallestValue = i;
        for (unsigned int j = i + 1; j < size; ++j) {
            if (theArray[j] < theArray[smallestValue]) {
                smallestValue = j;
            }
        }
        swapValues(theArray, i, smallestValue);
    }
}
 ...
 */
void swapValues (int values[], int index1, int index2) {
    unsigned int holder = values[index2];
