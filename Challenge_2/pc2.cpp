...
double launchHumanCannonball (double initialVelocity, double launchAngle)
{
	double radAngle(0);
	double flightTime(0);
	double finalXVeloc(0);
	double finalYVeloc(0);
	
	radAngle = (launchAngle * (M_PI)/180);
	finalXVeloc = initialVelocity * cos(radAngle);
	finalYVeloc = initialVelocity * sin(radAngle) * -1;
	flightTime = finalYVeloc * (2 / -9.8);
	return (finalXVeloc * flightTime);
